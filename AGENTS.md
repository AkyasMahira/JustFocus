...
- One Swift file per type (the target uses a `PBXFileSystemSynchronizedRootGroup`, so new files under `JustFocus/` are auto-registered — never hand-edit `project.pbxproj`).
  ...
  - `TaskDetail.swift` — detail destination pushed from `FocusView`; takes a `TaskItem` reference and renders description/due date/note. Pencil opens `AddTaskSheet(editingTask:)`; the checkmark toolbar button asks for confirmation, then completes the task (`isCompleted = true`), fires `onCompleted` (which may trigger `CongratsScreen`), and pops back to the list.
